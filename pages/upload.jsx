...
export default function Upload() {
    ...
    const upload = async () => {
        ...
        if (selectedFile?.type === "application/pdf") {
            ...
            setResume(resumeUrl);
            const aiData = await fetch(`${process.env.NEXT_PUBLIC_PYTHON_BACKEND_URL}`, {
                method: "POST",
                headers: {
            ...
            }).then(res => res.json()).catch(err => setShowSelectFile(true));
    ...
    return (
        <div>
            ...
            {!aiResponse ? <main>
                ...
                <div className="flex">
                    <button className="block bg-secondary px-4 py-2 ml-4 mt-8 rounded text-highlight disabled:opacity-50" disabled={processing} onClick={upload}>{processing ? "Uploading..." : "Upload"}</button>
                ...
                <main className="ml-4">
                    ...
                    <div className="text-primary text-xl my-4">Please verify the information we obtained from your resume.</div>
                    <div className="max-w-[50%]">
                        {typeof aiResponse === "object" ? Object.keys(aiResponse).map((k, index) => {
                            return <div key={index} className="flex flex-col mb-3">
                                <label className="text-xl font-bold">{`${k}: `}</label>
                                <input className="block w-full bg-secondary border-0 rounded ml-4 mt-2 px-4 py-2 cursor-pointer" type="text" value={aiResponse[k]} onChange={(e) => {
                    ...
                    <button className="block bg-secondary px-4 py-2 ml-4 mt-8 rounded text-highlight disabled:opacity-50" disabled={processing} onClick={async () => {
                        setProcessing(true);
