...
export default function Card() {
  ...
  useEffect(() => {
    console.log(user)
    if (user) handlePageLoad()
  }, [user])

  return (
    <>
      {user ? (
        <main className='overflow-y-auto bg-background h-screen'>
          <div className='ml-4 mt-24'>
            <h1 className='text-6xl font-bold mt-2 text-white'>Resume Data</h1>
            <button
              className='block bg-secondary px-4 py-2 mt-8 rounded text-highlight disabled:opacity-50'
              onClick={() =>
                router.push(`/users/${user.id}/businesscards/${cardId}`)
              }
              disabled={processing}
            >
              View Published Card
            ...
            <div className='max-w-[50%]'>
              {typeof resume === 'object'
                ? Object.keys(resume).map((k, index) => {
                    if (k === 'resume') return
                    return (
              ...
              <div className='flex'>
                <button
                  className='block bg-secondary px-4 py-2 mx-4 mt-8 rounded text-highlight disabled:opacity-50'
