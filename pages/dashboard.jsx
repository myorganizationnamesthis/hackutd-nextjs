...
export default function Dashboard() {
    ...
    return (
        <>
            ...
            <div className='overflow-y-auto bg-background h-screen block ml-4 md:mx-8'>
                {user ? (
                    <div className='mt-24 mx-auto block w-full relative block'>
                        ...
                        <div className="flex flex-wrap place-content-between md:gap-8">
                            {resumes.map((resume, index) => {
                                return (
                                    <button
                                        onClick={() => Router.push(`/cards/${resume.id}`)}
                                        key={index}
                                        className='bg-secondary rounded-lg p-4 mt-4 mr-6 w-[24rem] text-xl'
                                    >
                                        Card {index + 1}
                                        <iframe className='mx-auto' width={350} height={192} src={`/users/${user.id}/businesscards/${resume.id}`} />
                                    </button>
                                )
                            })}
                        </div>
